...
The supported Windows execution path is WSL2. The Windows desktop process launches the bridge through `wsl.exe`, so no HTTP server or open port is required and the same stdio protocol is used on every platform.
...
`shape-512` and `shape-1024` load only sparse-structure and shape models and skip texture generation. They retain raw triangle geometry and the shape latent while avoiding the texture flow model and texture decoder. Hardware qualification remains explicit through `probe`; a profile is never silently substituted.
